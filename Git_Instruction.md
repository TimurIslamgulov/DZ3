...
Чтобы загрузить в удаленный репозиторий все, что сделано в локальном, существует команда **git push**, например:
...
`git clone` - клонирует (копирует) репозиторий из гитхаб ([ссылка](github.com))
