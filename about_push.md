...
## Теперь поговорим о работе с удаленными репозиториями. Для этого мы будем использоваться крупнейшим онлайн сревисом GitHub
 

### 1. Мы можем создать копию репозитория, который находится по ссылке (ссылка на репозиторий).Также мы можем указать название новой папки, в которую Git скопирует репозиторий (название папки). Если его не указать, папка будет называться так же, как и сам репозиторий. Дл яэтого необходимо воспользоваться командпой:

>git clone <ссылка на репозиторий> <название папки>


### 2. Для того, чтобы создавать, просматривать и удалять подключения к другим репозиториям, необходима следующая команда:

>git remote

Следующая команда аналогична команде выше, но включает URL-адрес каждого подключения.

>git remote -v

Для создания нового подключения к удаленному репозиторию используют команду:

>git remote add < name > < url >

Для удаления подключения к удаленному репозиторию с именем ＜name＞ используют команду:

>git remote rm < name >

Для переименования удаленного подключения с имени ＜old-name＞ на ＜new-name＞ используют команду:

>git remote rename < old-name > < new-name >


### 3. Для выгрузки содержимого локального репозитория в удаленный репозиторий и передчи коммитов из локального репозитория в удаленный используют команду:

>git push

Для создания локальной ветки в репозитории назначения используют команду:

>git push < remote > < branch >  

Следующая команда аналогична приведенной выше команде, однако данные будут опубликованы принудительно, даже если нельзя выполнить ускоренное слияние

>git push < remote > --force

Для публикации всех локальных веток в указанном удаленном репозитории используют команду:
...
### 4. Для извлечения и загрузки содержимого из удаленного репозитория и немедленного обновления локального репозитория этим содержимым используют команду:

>git pull

Для извлечения из указанного удаленного репозитория копию текущей ветки и немедленно слить ее с локальной копией используют команду:

>git pull < remote >

Подобно вызову по умолчанию, для извлечения удаленного содержимого, без создания нового коммита со слитым содержимым используют команду:

>git pull --no-commit < remote >

## Далее представим инструкцию для создание pull request.
...
6. В окне github появзяется возможность отправить pull request.